...
export interface AiLifecycleResult {
  ubid: string;
  ai_narration: string;
...
export async function explainLifecycleAi(ubid: string): Promise<AiLifecycleResult> {
  const res = await fetch(`${BASE}/unified/${ubid}/explain-llm`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
...
export interface SmartQueryFilters {
  ...
  event_type?: string;
  no_event_since_months?: number;
}

export interface SmartQueryResultRow {
  ubid: string;
  canonical_name: string;
...
export interface SmartQueryResponse {
  ...
  ai_summary: string;
  results: SmartQueryResultRow[];
}

export async function runSmartQuery(filters: SmartQueryFilters): Promise<SmartQueryResponse> {
  const params = new URLSearchParams();
  for (const [k, v] of Object.entries(filters)) {
    if (v !== undefined && v !== '' && v !== null) params.set(k, String(v));
  }
  const res = await fetch(`${BASE}/insights/query?${params}`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export interface AiLinkageResult {
  linkage_id: string;
  ai_narration: string;
  score: number;
  confidence: string;
...
export async function explainLinkageAi(linkageId: string): Promise<AiLinkageResult> {
  const res = await fetch(`${BASE}/linkage/${linkageId}/explain-llm`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
