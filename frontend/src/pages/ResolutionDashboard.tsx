...
import { getLinkageResults, reviewLinkage, explainLinkageAi } from '../api';
...
export default function ResolutionDashboard() {
  ...
  const [aiByPair, setAiByPair] = useState<Record<number, string>>({});
  ...
  const toggleExpand = (id: number) => {
    ...
    // Lazy-load AI rationale on first expand
    if (!aiByPair[id]) {
      setAiLoading((prev) => new Set(prev).add(id));
      explainLinkageAi(String(id))
        .then((res) => setAiByPair((prev) => ({ ...prev, [id]: res.ai_narration })))
        .catch(() => setAiByPair((prev) => ({ ...prev, [id]: 'AI explanation unavailable. See feature scores below.' })))
        .finally(() => setAiLoading((prev) => {
          const next = new Set(prev);
          next.delete(id);
          return next;
        }));
                  ...
                  <div className="border-t border-gray-100 px-5 py-4 bg-gray-50 space-y-4">
                    ...
                    <div className="rounded-lg bg-indigo-50 border border-indigo-100 p-3">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] uppercase tracking-wider text-indigo-700 font-bold">AI Linkage Rationale</span>
                      ...
                      <p className="text-sm text-gray-700">
                        {aiLoading.has(pair.id) ? (
                          <span className="text-gray-400">Generating rationale…</span>
                        ) : (
                          aiByPair[pair.id] ?? '—'
                        )}
                      </p>
