...
from app.services.llm_narration import explain_lifecycle
...
@router.get("/{ubid}/explain-llm")
def explain_with_llm(ubid: str, db: Session = Depends(get_db)):
    """AI-grounded lifecycle narration (Azure GPT-4.1).

    Detects source-system conflicts (e.g., active in GST but closed in MCA), summarises
    the strongest signal, and recommends the next data-stewardship action.
    """
    entity = db.query(UnifiedBusiness).filter(
        (UnifiedBusiness.ubid == ubid) | (UnifiedBusiness.id == ubid)
    ).first()
    ...
    statuses = {s["status_in_source"] for s in sources if s["status_in_source"]}
    has_conflict = len(statuses) > 1

    events = [
        {
            "type": ev.event_type,
            "timestamp": ev.event_date.isoformat() if ev.event_date else None,
    ...
    payload = {
        "ubid": entity.ubid,
        "business_name": entity.canonical_name,
    ...
    return {
        "ubid": entity.ubid,
        "ai_narration": explain_lifecycle(payload),
        "has_conflict": has_conflict,
        "conflicting_statuses": sorted(statuses) if has_conflict else [],
